...
class ImageModel(BaseModel):
    ...
    def text_to_image(self, action):
        try:
            self.log_info("ImageModel", "Generating image from text")
            
            #sdp_builder = SDPBuilder()
            #sdp_builder.text_to_image(action).save(action)
        except Exception as e:
            self.log_error("ImageModel", f"Error in text_to_image: {e}")

    def image_to_image(self, action):
        try:
            self.log_info("ImageModel", "Generating image from text and a base image")
            
            #sdp_builder = SDPBuilder()
            #sdp_builder.image_to_image(action).save(action)
        except Exception as e:
            self.log_error("ImageModel", f"Error in image-to-image: {e}")

    def draw_text(self, action):
        try:
            self.log_info("ImageModel", "Drawing text over image")
            
            #sdp_builder = SDPBuilder()
            #sdp_builder.load(action).draw_text(action).save(action)
        except Exception as e:
            self.log_error("ImageModel", f"Error in draw-text: {e}")

    def color_to_image(self, action):
        try:
            self.log_info("ImageModel", "Creating image from color")
            
            #sdp_builder = SDPBuilder()
            #sdp_builder.color_to_image(action).save(action)
        except Exception as e:
            self.log_error("ImageModel", f"Error in color-to-image: {e}")

    def resize_image(self, action):
        try:
            self.log_info("ImageModel", "Resizing image")
            
            #sdp_builder = SDPBuilder()
            #sdp_builder.load(action).resize(action).save(action)
        except Exception as e:
            self.log_error("ImageModel", f"Error in resize-image: {e}")

    def paste_image(self, action):
        try:
            self.log_info("ImageModel", "Inserting an image into another")
            
            #sdp_builder = SDPBuilder()
            #sdp_builder.load(action).insert_image(action).save(action)
        except Exception as e:
            self.log_error("ImageModel", f"Error in paste-image: {e}")

    def with_speech_bubbles(self, action):
        try:
            self.log_info("ImageModel", "Inserting speech bubbles")
            
            #sdp_builder = SDPBuilder()
            #sdp_builder.load(action).apply_speech_bubbles(action).save(action)
        except Exception as e:
            self.log_error("ImageModel", f"Error in with-speech-bubbles: {e}")
